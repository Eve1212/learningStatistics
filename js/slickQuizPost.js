 ...
 * @license MIT
 */
var selectedAnswers = [], trueAnswers = [], postTrueAnswer = [], outputselectedAnswers = [],selectedPostAnswer = [], totalCorrectAnswers = 0,  posttestCorrect = 0;
(function($){
    $.slickQuizPost = function(element, options) {
        var plugin   = this,
            $element = $(element),
            ...
            _quizNamePost             = _element + ' .quizNamePost',
        ...
        var quizValues = (plugin.config.json ? plugin.config.json : typeof posttest != 'undefined' ? posttest : null);
        ...
        plugin.method = {
            ...
            setupQuiz: function(options) { // use 'options' object to pass args
                ...
                $quizNamePost.hide().html(plugin.config.nameTemplateText
              ...
              //  var trueAnswers = [];
               ...
               // var selectedAnswers = [];
                answerSelects.each( function() {
                    ...
                    outputselectedAnswers.push(parseInt(id.replace(/(.*\_question\d{1,}_)/, ''), 10)+1);
                    selectedPostAnswer.push(parseInt(id.replace(/(.*\_question\d{1,}_)/, ''), 10));
                });

                if (plugin.config.preventUnanswered && selectedPostAnswers.length === 0) {
                    alert(plugin.config.preventUnansweredText);
                    return false;
                }

                // Verify all/any true answers (and no false ones) were submitted
                var correctResponse = plugin.method.compareAnswers(postTrueAnswer, selectedPostAnswer, selectAny);
            ...
            backToQuestion: function(backButton, options) {
                ...
                trueAnswers.pop();
                selectedPostAnswer.pop();
                selectedAnswers.pop();
                outputselectedAnswers.pop();
                postTrueAnswer.pop();
            ...
            completeQuiz: function(options) {
                ...
                for (i = 0; i < questions.length; i++){
                    if (postTrueAnswer[i] == selectedPostAnswer[i]){
                        totalCorrectAnswers = totalCorrectAnswers + 1;
                        posttestCorrect = posttestCorrect + 1;
                    }
                }

                var score = posttestCorrect, displayScore = posttestCorrect;
...
/*
               if (selectedPostAnswer.length == NumberOfPretestQuestions){
                   $('<br />').insertBefore('#btn_finish');

                    document.getElementById("btn_finish").style.visibility = "visible"
                ...
                $quizArea.fadeOut(300, function() {
                    ...
                    $('#slickQuizPost').append("<br>").append(Completion());
                });
            ...
            compareAnswers: function(postTrueAnswer, selectedPostAnswer, selectAny) {
                if ( selectAny ) {
                    return $.inArray(selectedPostAnswer[0], postTrueAnswer) > -1;
                } else {
                    // crafty array comparison (http://stackoverflow.com/a/7726509)
                    return ($(postTrueAnswer).not(selectedPostAnswer).length === 0 && $(selectedPostAnswer).not(postTrueAnswer).length === 0);
                }
            ...
            calculateLevel: function(totalCorrectAnswers) {
                var percent = (totalCorrectAnswers / questionCount).toFixed(2),
                    level   = null;

                if (plugin.method.inRange(0, 0.17, percent)) {
                    level = 5;
                } else if (plugin.method.inRange(0.18, 0.34, percent)) {
                    level = 4;
                } else if (plugin.method.inRange(0.35, 0.50, percent)) {
                    level = 3;
                } else if (plugin.method.inRange(0.51, 0.67, percent)) {
                    level = 2;
                } else if (plugin.method.inRange(0.68, 0.84, percent)) {
                    level = 1;
                } else if (plugin.method.inRange(0.85, 1.00, percent)) {
                    level = 0;
                }
                return level;
            },
    ...
    $.fn.slickQuizPost = function(options) {
        return this.each(function() {
            if (undefined === $(this).data('slickQuizPost')) {
                var plugin = new $.slickQuizPost(this, options);
                $(this).data('slickQuizPost', plugin);
            }
        });
    };
})(jQuery);
